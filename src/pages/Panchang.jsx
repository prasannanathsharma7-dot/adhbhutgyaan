...
export default function Panchang() {
    ...
    // 1. AUTOMATIC BROWSER GEOLOCATION ON PAGE LOAD
    useEffect(() => {
        if (typeof window === 'undefined' || !navigator.geolocation) {
            setGpsStatus('denied');
            setGpsNotice(t('डिफ़ॉल्ट रूप से वाराणसी (काशी) पंचांग प्रदर्शित हो रहा है।', 'Defaulting to Varanasi (Kashi). Search your city above.'));
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                ...
                // Detect local timezone offset in hours
    ...
    // 2. GLOBAL CITY SEARCH (NOMINATIM DEBOUNCE)
    useEffect(() => {
    ...
    const handleSelectLocation = (item) => {
        ...
        // Approximate timezone from longitude (lng / 15) or user local offset
    ...
    // 3. COMPUTE REAL-TIME DYNAMIC PANCHANG
    ...
    useSEO({
        title: t(
            `आज का पंचांग — ${panchangData.location.city} (${panchangData.dateFormatted}) | शुभ मुहूर्त एवं राहु काल`,
            `Today's Panchang — ${panchangData.location.city} (${panchangData.dateFormatted}) | Shubh Muhurat & Choghadiya`
        ),
        description: t(
            `${panchangData.location.city} का दैनिक वैदिक पंचांग: तिथि ${panchangData.tithi.name}, नक्षत्र ${panchangData.nakshatra.name}, अभिजित मुहूर्त ${panchangData.muhurats.abhijit}, राहु काल ${panchangData.inauspicious.rahuKaal} एवं चौघड़िया।`,
            `Vedic Panchang for ${panchangData.location.city}: Tithi ${panchangData.tithi.name}, Nakshatra ${panchangData.nakshatra.name}, Abhijit Muhurat ${panchangData.muhurats.abhijit}, Rahu Kaal ${panchangData.inauspicious.rahuKaal} & Choghadiya.`
        ),
        path: '/panchang',
...
📍 *${panchangData.location.city}, ${panchangData.location.country}*
📅 *${panchangData.dateFormatted}*
────────────────────────────
📜 *TITHI:* ${panchangData.tithi.name} (${panchangData.tithi.paksha})
⭐ *NAKSHATRA:* ${panchangData.nakshatra.name} (Lord: ${panchangData.nakshatra.lord})
✨ *YOGA:* ${panchangData.yoga.name} | *KARANA:* ${panchangData.karana.name}
☀️ *SURYA RASHI:* ${panchangData.transits.suryaRashi}
🌙 *CHANDRA RASHI:* ${panchangData.transits.chandraRashi}

⏰ *LOCAL SOLAR TIMINGS:*
• Sunrise: ${panchangData.solar.sunrise} | Sunset: ${panchangData.solar.sunset}
• Solar Noon: ${panchangData.solar.solarNoon}

🟢 *SHUBH MUHURAT:*
• *Abhijit Muhurat:* ${panchangData.muhurats.abhijit} ✅
• *Brahma Muhurat:* ${panchangData.muhurats.brahma} 🧘
• *Amrit Kaal:* ${panchangData.muhurats.amritKaal} ✨

🔴 *ASHUBH KAAL (VARJIT):*
• *Rahu Kaal:* ${panchangData.inauspicious.rahuKaal} ⏳
• *Yamaganda:* ${panchangData.inauspicious.yamaganda}
• *Gulika Kaal:* ${panchangData.inauspicious.gulikaKaal}

🙏 *DAILY CHANT:*
"${panchangData.vara.dailyChant}" (108 times)
            ...
            <div className="container" style={{ marginTop: '-1.5rem', position: 'relative', zIndex: 10 }}>
                    ...
                    {gpsNotice && (
                        <div style={{ marginTop: '0.75rem', paddingTop: '0.65rem', borderTop: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem', fontSize: '0.82rem', color: location.source === 'gps' ? '#065f46' : 'var(--text-secondary)' }}>
                            ...
                            <div style={{ color: 'var(--gold-800)', fontWeight: 600 }}>
                                🌐 Lat: {panchangData.location.latitude}° | Lon: {panchangData.location.longitude}° | {panchangData.location.timezoneOffset}
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
                    <div style={{ background: 'white', padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-sm)', textAlign: 'center' }}>
                        ...
                        <strong style={{ fontSize: '1.25rem', color: '#ea580c', display: 'block', marginTop: '0.2rem' }}>☀️ {panchangData.solar.sunrise}</strong>
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>Day Length: {panchangData.solar.dayLength}</span>
                    </div>

                    <div style={{ background: 'white', padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-sm)', textAlign: 'center' }}>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', display: 'block' }}>{t('सूर्यास्त', 'Local Sunset')}</span>
                        <strong style={{ fontSize: '1.25rem', color: '#7c2d12', display: 'block', marginTop: '0.2rem' }}>🌅 {panchangData.solar.sunset}</strong>
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>Solar Noon: {panchangData.solar.solarNoon}</span>
                    </div>

                    <div style={{ background: 'white', padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-sm)', textAlign: 'center' }}>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', display: 'block' }}>{t('दिन एवं देवता', 'Vara & Deity')}</span>
                        <strong style={{ fontSize: '1.15rem', color: panchangData.vara.themeColor, display: 'block', marginTop: '0.2rem' }}>{panchangData.vara.name}</strong>
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>{panchangData.vara.deity}</span>
                    </div>
                    ...
                    <div style={{ background: 'white', padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-sm)', textAlign: 'center' }}>
                        ...
                        <strong style={{ fontSize: '0.95rem', color: 'var(--navy-900)', display: 'block', marginTop: '0.2rem' }}>☀️ {panchangData.transits.suryaRashi.split(' ')[0]}</strong>
                        <span style={{ fontSize: '0.75rem', color: 'var(--gold-800)', fontWeight: 600 }}>🌙 {panchangData.transits.chandraRashi.split(' ')[0]}</span>
                    </div>
                </div>
                    ...
                    <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: '1.5rem', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-md)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0.85rem', background: 'var(--warm-50)', borderRadius: 'var(--radius-md)' }}>
                                ...
                                <span style={{ fontWeight: 700, color: 'var(--navy-900)', fontSize: '0.9rem' }}>{panchangData.tithi.name} ({panchangData.tithi.paksha})</span>
                            </div>

                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0.85rem', background: 'var(--warm-50)', borderRadius: 'var(--radius-md)' }}>
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}><b>⭐ {t('नक्षत्र', 'Nakshatra')}:</b></span>
                                <span style={{ fontWeight: 700, color: 'var(--navy-900)', fontSize: '0.9rem' }}>{panchangData.nakshatra.name} (Pada {panchangData.nakshatra.pada}) · {panchangData.nakshatra.lord}</span>
                            </div>

                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0.85rem', background: 'var(--warm-50)', borderRadius: 'var(--radius-md)' }}>
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}><b>✨ {t('योग', 'Yoga')}:</b></span>
                                <span style={{ fontWeight: 700, color: 'var(--navy-900)', fontSize: '0.9rem' }}>{panchangData.yoga.name}</span>
                            </div>

                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0.85rem', background: 'var(--warm-50)', borderRadius: 'var(--radius-md)' }}>
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}><b>🪷 {t('करण', 'Karana')}:</b></span>
                                <span style={{ fontWeight: 700, color: 'var(--navy-900)', fontSize: '0.9rem' }}>{panchangData.karana.name}</span>
                            </div>

                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0.85rem', background: 'var(--warm-50)', borderRadius: 'var(--radius-md)' }}>
                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}><b>🕉️ {t('वार (दिन)', 'Vara / Day')}:</b></span>
                                <span style={{ fontWeight: 700, color: 'var(--navy-900)', fontSize: '0.9rem' }}>{panchangData.vara.name}</span>
                            </div>
                        </div>
                        ...
                        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 'var(--radius-lg)', padding: '1.25rem', boxShadow: 'var(--shadow-sm)' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.6rem', fontSize: '0.85rem' }}>
                                <div style={{ background: 'white', padding: '0.6rem', borderRadius: 'var(--radius-md)', border: '1px solid #bbf7d0' }}>
                                    ...
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem' }}>{panchangData.muhurats.abhijit}</div>
                                </div>
                                <div style={{ background: 'white', padding: '0.6rem', borderRadius: 'var(--radius-md)', border: '1px solid #bbf7d0' }}>
                                    <div style={{ fontSize: '0.72rem', color: '#15803d', fontWeight: 700 }}>ब्रह्म मुहूर्त (Brahma)</div>
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem' }}>{panchangData.muhurats.brahma}</div>
                                </div>
                                <div style={{ background: 'white', padding: '0.6rem', borderRadius: 'var(--radius-md)', border: '1px solid #bbf7d0' }}>
                                    <div style={{ fontSize: '0.72rem', color: '#15803d', fontWeight: 700 }}>अमृत काल (Amrit Kaal)</div>
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem' }}>{panchangData.muhurats.amritKaal}</div>
                                </div>
                                <div style={{ background: 'white', padding: '0.6rem', borderRadius: 'var(--radius-md)', border: '1px solid #bbf7d0' }}>
                                    <div style={{ fontSize: '0.72rem', color: '#15803d', fontWeight: 700 }}>गोधूलि मुहूर्त (Godhuli)</div>
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem' }}>{panchangData.muhurats.godhuli}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 'var(--radius-lg)', padding: '1.25rem', boxShadow: 'var(--shadow-sm)' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', fontSize: '0.8rem' }}>
                                <div style={{ background: 'white', padding: '0.6rem 0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid #fecaca', textAlign: 'center' }}>
                                    ...
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem', fontSize: '0.78rem' }}>{panchangData.inauspicious.rahuKaal}</div>
                                </div>
                                <div style={{ background: 'white', padding: '0.6rem 0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid #fecaca', textAlign: 'center' }}>
                                    <div style={{ fontSize: '0.7rem', color: '#b91c1c', fontWeight: 700 }}>यमगण्ड</div>
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem', fontSize: '0.78rem' }}>{panchangData.inauspicious.yamaganda}</div>
                                </div>
                                <div style={{ background: 'white', padding: '0.6rem 0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid #fecaca', textAlign: 'center' }}>
                                    <div style={{ fontSize: '0.7rem', color: '#b91c1c', fontWeight: 700 }}>गुलिक काल</div>
                                    <div style={{ fontWeight: 700, color: '#0f172a', marginTop: '0.15rem', fontSize: '0.78rem' }}>{panchangData.inauspicious.gulikaKaal}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: '1.5rem', border: '1px solid var(--border-light)', boxShadow: 'var(--shadow-md)', marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.75rem' }}>
                        {(choghadiyaTab === 'day' ? panchangData.choghadiya.day : panchangData.choghadiya.night).map((slot, i) => (
                            <div
                                key={i}
                                style={{
                                    padding: '0.85rem',
