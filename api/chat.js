// AI chatbot for the website: answers questions about services/pricing/timing
// using the SAME data as the rest of the site (src/data/services.json), and
// can create a real booking (writes to the same `bookings` collection the
// booking form uses) once it has collected name + phone + service.
//
// Requires one env var in Vercel: ANTHROPIC_API_KEY (from console.anthropic.com).
// Uses Claude Haiku - fast and inexpensive, which fits a high-volume FAQ/booking
// bot much better than a larger model.
...
6. Keep replies short and warm (2-4 sentences). Reply in the same language the person writes in (Hindi, Hinglish, or English).
7. For anything you're unsure about or that isn't in the facts above (exact pricing, availability on a specific date, custom requests), say the team will confirm this directly and offer the WhatsApp number - never guess.`;
}
...
async function createBookingFromTool(db, input, req) {
    const doc = {
    ...
    notifyAdmin({
        emailSubject: `🤖 New Chatbot Booking - ${doc.name}`,
        emailHtml: `
            <h2>New Booking via Website Chatbot</h2>
            ...
            <p style="color:#888;font-size:12px;">Booking ID: ${result.insertedId} (via chatbot)</p>
        `,
        whatsappText: `🤖 New Chatbot Booking\n\nName: ${doc.name}\nPhone: ${doc.phone}\nService: ${doc.serviceName || '-'}\nMode: ${doc.mode || '-'}\nDate: ${doc.preferredDate || 'To be decided'}`,
    });
    ...
    if (doc.email) {
        sendMail({
            ...
            html: `
                ...
                <p>We have received your booking enquiry for <b>${escapeHtml(doc.serviceName) || 'a pooja'}</b> via our website chat.</p>
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Keep it to plain {role, content} pairs from the client; trim history size.
        ...
        let response = await callAnthropic(messages, process.env.ANTHROPIC_API_KEY);
        let bookingCreated = null;

        // Handle at most one round of tool use (create_booking), then get the
        // final natural-language reply. Keeps the chatbot's own latency and
        // cost bounded to two API calls max per user message.
        if (response.stop_reason === 'tool_use') {
            ...
            if (toolUseBlock && toolUseBlock.name === 'create_booking') {
                const result = await createBookingFromTool(db, toolUseBlock.input || {}, req);
            ...
            response = await callAnthropic(followupMessages, process.env.ANTHROPIC_API_KEY);
        }
        ...
        res.status(200).json({ ok: true, reply: replyText, bookingCreated: Boolean(bookingCreated) });
    } catch (err) {
